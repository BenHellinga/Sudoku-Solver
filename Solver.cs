...
// only works on the specific color scheme of https://sudoku.com/
...
internal class Solver
{
    ...
    static List<List<int>> board = null!;
    ...
    static ScreenReader s = null!;

    // per digit, pixel offsets within a tile that are only ever background when that digit isn't present
    static List<List<Point>> NUMBER_RESTRICTIONS = null!;
    ...
    static void Main()
    {
        ...
        Console.WriteLine("Press enter to save cursor position");
        Console.ReadKey();
        ...
        gameLocation = CursorManager.findColorNearPoint(LINE_COLOR, cursorPosition, SCAN_SIZE, SCAN_SIZE);

        // dismiss the difficulty/new game popups before reading the board
    ...
    // captures the board area and reads every tile into the board array
    ...
    // figures out which digit (if any) is in tile (i, j), leaves it as 0 if blank
    static void parseNumber(int i, int j)
    {
        ...
        for (int k = 8; k >= 0; k--)
        {
            ...
            // if any restriction pixel for digit k+1 is a background color, it can't be that digit
    ...
    // fills the 9x9 board with zeros (blank tiles)
    ...
    // hardcoded pixel offsets used to tell digits 1-9 apart within a tile
    ...
    // recursive backtracking solve, t is the tile index (0-80)
    ...
    // standard sudoku rules, checks row, column, and 3x3 box for a conflict
    ...
    // types the solved digits into the page, snaking left-right-left through rows
